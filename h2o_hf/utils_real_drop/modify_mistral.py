...
__all__ = ["H2OMistral3ForConditionalGeneration", "H2OMistral3Attention",
           "H2OMistral3Attention_streaming", "H2OMistral3ForConditionalGeneration_streaming"]
...
class Mistral3Config(PretrainedConfig):
    ...
    def __init__(
        ...
        image_token_index=10,        vision_config=None,
        text_config=None,
...
def _make_causal_mask(
    bsz: int, tgt_len: int, past_key_values_length: int, 
...
def apply_rotary_pos_emb_single(x, cos, sin, position_ids):
    ...
    sin = sin.squeeze(1).squeeze(0)    cos = cos[position_ids].unsqueeze(1)
...
class H2OKVCache_LayerWise:
    ...
    def __call__(self, past_key_values, attn_score_cache):
        ...
        keep_recent = torch.arange(
            seq_len - self.recent_size, seq_len, 
        ...
        v_hh_recent = past_key_values[1].squeeze()[mask].view(
    ...
    def evict_for_space(self, past_key_values, num_coming):
        ...
        keep_recent = torch.arange(
            seq_len - self.recent_size + num_coming, seq_len, 
            device=keep_topk.device
        ).repeat(keep_topk.shape[0], 1)
        keep_idx = torch.cat([keep_topk, keep_recent], dim=-1)
        ...
        k_hh_recent = past_key_values[0].squeeze()[mask].view(
            bsz, num_heads, -1, head_dim
        )
        v_hh_recent = past_key_values[1].squeeze()[mask].view(
            bsz, num_heads, -1, head_dim
    ...
    def _update_hh_score(self, attn_score_cache):
        ...
        if self.hh_score is None:            self.hh_score = attn_score_cache.sum(0).sum(1)
        else:
...
class H2OMistral3Attention(nn.Module):
    ...
    def forward(
        ...
        if position_ids is not None and position_ids.nelement() > 1:
            if position_length < position_ids.max().item() + 1:
                position_length = position_ids.max().item() + 1
...
class H2OMistral3ForConditionalGeneration(MistralForCausalLM):
    def __init__(self, config):
        super().__init__(config)
        num_layers = len(self.model.layers)
        for layer_idx in range(num_layers):
            self.model.layers[layer_idx].self_attn = H2OMistral3Attention(config)


class H2OMistral3Attention_streaming(nn.Module):
    ...
    def __init__(self, config: Mistral3Config):
        ...
        self.head_dim = self.hidden_size // self.num_heads
        self.num_key_value_heads = config.num_key_value_heads
        self.num_key_value_groups = self.num_heads // self.num_key_value_heads        self.max_position_embeddings = config.max_position_embeddings
        self.rope_theta = config.rope_theta
...
class H2OMistral3ForConditionalGeneration_streaming(MistralForCausalLM):
    def __init__(self, config):
        super().__init__(config)
        num_layers = len(self.model.layers)
        for layer_idx in range(num_layers):
            self.model.layers[layer_idx].self_attn = H2OMistral3Attention_streaming(config)
